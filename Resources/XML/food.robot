...
Add New Food
    ...
    Add Element  ${root}  <name>Grilled Cheese Sandwich</name>  xpath=food[7]
    Add Element  ${root}  <price>$2455</price>  xpath=food[7]
    Add Element  ${root}  <description>this is slices</description>  xpath=food[7]
    Add Element  ${root}  <calories>4200</calories>  xpath=food[7]
    Save Xml  ${root}  ${XML_MENU_PATH}

Verify New Food Exists
    Element Should Exist  ${XML_MENU_PATH}  food[7]